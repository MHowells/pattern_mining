...
def get_n(q, pathway_matrix):
    ...
    return pathway_matrix[:, :, q].sum()


def get_pi(q, z, pathway_matrix):
    """ 
    Gets pi(q, z), the probability of leaving state q via letter z.
    """
    return pathway_matrix[z, q, :].sum() / get_n(q, pathway_matrix)


def get_pi_endpoint(q, pathway_matrix, alphabet):
    """
    Gets pi(q), the probability of terminating at state q.
    """
    return 1 - sum(get_pi(q, z, pathway_matrix) for z in alphabet)


def hoeffding_bound(q1, q2, alpha, pathway_matrix, alphabet):
    """ 
    Returns a Boolean indicating whether the Hoeffding bound is satisfied.
    """
    alpha_constant = (np.log(2 / alpha) / 2) ** 0.5
    rhs = np.sqrt(1/2 * np.log(2/0.05)) * ((1/np.sqrt(get_n(q1, pathway_matrix))) + (1/np.sqrt(get_n(q2, pathway_matrix))))
    for z in alphabet:
        lhs = abs(get_pi(q1, z, pathway_matrix) - get_pi(q2, z, pathway_matrix))
        if lhs > rhs:
            return False
    lhs = abs(get_pi_endpoint(q1, pathway_matrix, alphabet) - get_pi_endpoint(q2, pathway_matrix, alphabet))
    if lhs > rhs:
        return False
    return True
...
def merge_two_states(q1, q2, pathway_matrix, states):
    ...
    new_column = pathway_matrix_copy[:, :, q1] + pathway_matrix_copy[:, :, q2]
    pathway_matrix_copy = np.dstack((np.delete(pathway_matrix_copy, [q1, q2], 2), new_column))
    new_row = pathway_matrix_copy[:, q1, :] + pathway_matrix_copy[:, q2, :]
    pathway_matrix_copy = np.dstack((np.delete(pathway_matrix_copy, [q1, q2], 1).transpose(0, 2, 1), new_row)).transpose(0, 2, 1)
